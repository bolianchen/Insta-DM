...
def demo_visualize(args, demo_loader, disp_net, ego_pose_net, obj_pose_net):
    ...
    for i, (ref_img, tgt_img, ref_seg, tgt_seg, intrinsics, intrinsics_inv) in enumerate(demo_loader):
        ...
        _, _, r2t_ego_projected_depth, r2t_ego_computed_depth = inverse_warp2(
                ref_img, tgt_depth, ego_pose, intrinsics, ref_depth)
        ...
        if num_inst != 0:
            ...
            i_w = ((IMDDs[sq][0]) * 0.5 + 0.5)[bb].detach().cpu().numpy().transpose(1,2,0)
            i_w_masked = i_w
            d_diff = ( ((IMDDs[sq][3]) - (IMDDs[sq][2])).abs() / ((IMDDs[sq][3]) + (IMDDs[sq][2])).abs().clamp(min=1e-3) ).clamp(0,1)[bb,0].detach().cpu()
            d_diff_ego = ( (r2t_ego_projected_depth - r2t_ego_computed_depth).abs() / (r2t_ego_projected_depth + r2t_ego_computed_depth).abs().clamp(min=1e-3) ).clamp(0,1)[bb,0].detach().cpu() * (IMDDs[sq][1])[bb,0].detach().cpu()
